...
removeDtoverlay() {
    dtoverlay -r imx519>/dev/null 2>&1
    dtoverlay -r arducam>/dev/null 2>&1
    dtoverlay -r arducam-pivariety>/dev/null 2>&1
    dtoverlay -r arducam_64mp>/dev/null 2>&1
    dtoverlay -r arducam-64mp>/dev/null 2>&1
}
...
if [[ $package == *"kernel_driver"* ]]; then
    ...
    #     echo ""
    #     ln -s /usr/lib/arm-linux-gnueabihf/libboost_program_options.so /usr/lib/arm-linux-gnueabihf/libboost_program_options.so.1.67.0
    # fi
fi
